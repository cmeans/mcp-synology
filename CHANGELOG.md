...
- **`get_file_info` now returns one record per input path on real DSM 7.x** (#TBD) — partial fix for #68 (the `get_file_info` half). `modules/filestation/metadata.py:58` called `SYNO.FileStation.List getinfo` without pinning to v2. On real DSM 7.x (which advertises v3) the request fell through to v3, which silently misinterprets the comma-joined `path` query param as a single literal path, returns one synthetic record with that bogus path, and the handler's `len(files) == 1` branch then renders it as a single info card — even though the caller asked about multiple paths. Same v2-pin pattern Delete/CopyMove/Search were already using; getinfo was missed when that sweep happened. Bumped `tests/conftest.py` `SYNO.FileStation.List` `max_version` from 2 to 3 (matches DSM 7.x reality) and added `test_request_pinned_to_v2` that asserts `version=2` lands on the wire — without the production fix, the test fails because the request would default to max_version=3. Also added `TestMultiPathDelete` integration test in `tests/test_integration.py` (creates two folders, deletes them in one multi-path call, verifies via `list_files` that both are actually gone) for use against a real NAS. The **`delete_files` half of #68** is undiagnosed locally — Delete IS already pinned to v2, respx-level wire-dump shows correct comma-joined `path`, and the existing unit test passes. The bug only manifests on the user's actual DSM 7.1.1; debug-level logs (`SYNOLOGY_LOG_LEVEL=debug`) on a reproducer are the next step. #68 stays open with that note pinned.
