...
- **Tidy-up bundle: seven small items from the 2026-04-16 review** (#92) — addresses 7 of 8 items on #45.
  ...
  **Deferred (item 6 of 8): `flock` on `~/.local/state/mcp-synology/global.yaml`.** The issue framed it as a one-line addition, but the actual race is the read-modify-write lost-update problem (main process + background update task + manual `--check-update` each independently load → mutate → save). `atomic_write_text` (#69) already prevents torn writes via `os.replace`; serialising only the save half doesn't prevent lost updates, and a proper fix needs callers to hold the lock across the load-modify-save sequence (or a higher-level `update_field` API). Out of scope for a "tidy-up" PR. Filing the deeper restructure as a follow-up; the practical risk on a single-user box stays low because the racing windows are brief.
